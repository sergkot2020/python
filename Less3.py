...
with open('salary.txt', 'r', encoding='UTF-8') as file:
    ...
    for line in file:
        ...
        file_salary.append(str(int(int(salary_string[len(salary_string)-2])*0.87)))
...
def game (name1, name2):
    with open(name1+'.txt', 'r', encoding='UTF-8') as file1:
        dict_1 = {}
        dict_2 = {}
        for line in file1:
    ...
    with open(name2+'.txt', 'r', encoding='UTF-8') as file2:
        for line in file2:
            key, value, perenos = line.split(' ')
            dict_2[key] = value
            if key != 'name':
                dict_2[key] = float(value)
    return dict_1, dict_2

dict_1, dict_2 = game(name, 'Devil')
